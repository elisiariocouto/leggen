...
- [x] `leggen --help` crashes with `FileNotFoundError` when config is missing, and silently creates a DB — route modules instantiate `NotificationService()`/`SyncService()` at import time (`routes/notifications.py:17`, `routes/sync.py:17`, `sync_service.py:35`). Fixed: `BackgroundScheduler` now creates its services lazily (properties with setters for test injection), `routes/sync.py`/`routes/notifications.py` no longer instantiate services at import, and the `--config` eager callback skips loading for `--help`/`-h` so subcommand help works without a config. Subprocess regression tests assert help exits 0 and creates no files.
- [x] `generate_auth_config` requires an already-valid config (group-level eager validation, `main.py:79-91`) — chicken-and-egg; same for `generate_sample_db`. Fixed: the eager `--config` callback treats these two bootstrap commands as config-optional — a present, valid config still loads, but a missing or invalid one falls back to an empty `ctx.obj` instead of exiting. Covered by subprocess tests (including the dashed `generate-auth-config` form).
- [x] Frontend treats success as failure: "Backup Now" checks `response.success` but backend returns `{completed: true}` (`Settings.tsx:126-131`, `backup.py:196`); S3 test checks `success` vs `{connected: true}` (`S3BackupConfigDrawer.tsx:77-84`, `backup.py:228`). Fixed: frontend now checks `response.completed` / `response.connected`, and the phantom `success?`/`message?` fields were removed from the `api.ts` return types (failures arrive as HTTP errors via `onError`, never in the success payload).
- [x] Disabled state can never be saved: Save button `disabled` includes `!config.enabled` in all three config drawers (`DiscordConfigDrawer.tsx:164`, `TelegramConfigDrawer.tsx:186`, `S3BackupConfigDrawer.tsx:237`). Fixed: Save now only disables while the mutation is pending; additionally `PUT /backup/settings` skips the S3 connection test when `enabled: false`, so a config with broken credentials can still be turned off (test added).
...
- [x] Fix wrong env var in error message: `LEGGEN_CONFIG` → `LEGGEN_CONFIG_FILE` (`utils/config.py:191`). Fixed alongside the `--help`-without-config item.
