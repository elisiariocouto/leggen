...
- `leggen/services/` — business logic: `sync_service.py` (orchestrator), `enablebanking_service.py` (external API; auth is an RS256 JWT signed with the RSA key at config `key_path`), `notification_service.py`, `backup_service.py`, `data_processors.py`, `rules/` (category rules: `lua_runtime.py` is the sandboxed Lua runtime, `engine.py` runs rules over transactions, `defaults.py` the builtin rules)
- `leggen/repositories/` — raw SQL against SQLite via stdlib `sqlite3` (no ORM). `db.py` provides the connection context manager. Schema lives entirely in `repositories/migrations/` (no Alembic): `run_migrations()` applies the ordered `MIGRATIONS` list in `_steps.py`, stamping `PRAGMA user_version` inside each migration's transaction. Version 1 is the baseline schema as of release 2025.9.22, the oldest database that can be upgraded; older ones are refused with `UnsupportedDatabaseVersionError`. To change the schema, append a `Migration` to that list — write it to be safe to re-run (see `_helpers.py` for `add_column_if_missing` and friends), never edit an existing one
...
- API types are **generated** from the backend: `frontend/src/types/api.gen.ts` comes from the committed `openapi.json` via openapi-typescript — run `just generate-api` after changing the Pydantic models in `leggen/api/models/` (CI fails if either file is stale). `frontend/src/types/api.ts` is a hand-written shim re-exporting friendly names (plus the generic `PaginatedResponse<T>`, which OpenAPI can't express)
